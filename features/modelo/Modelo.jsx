// MODELO — vehicle detail page
// Sections: Hero, Descripción/Specs, Colores, Galería/Características, Seguridad, Specs table, Cotización form
const ModeloHero = () => {
  // Detectar modelo desde el body
  const modelo = document.body.getAttribute('data-modelo') || 'S06';
  ...
  return (
    <section className="mhero">
      <img className="mhero__bg" src="assets/modelo-hero.jpg" alt="SOUEAST S06" />
      <div className="mhero__overlay" />
      <div className="mhero__content">
        <h1 className="mhero__brand">SOUEAST <span>S06</span></h1>
        <p className="mhero__tag">Más estilo</p>
        <div className="mhero__ctas">
          ...
          <CTA variant="orange" href="cotizador.html?modelo=S06" onClick={handleCotizar}>Cotizar</CTA>
        </div>
      </div>
    <style>{`
...
const DescripcionSection = () => {
  const [version, setVersion] = React.useState("1.5T-6DCT");
  const [open, setOpen] = React.useState(false);
  return (
    <section className="desc">
      <img className="desc__img" src="assets/modelo-side.jpg" alt="SOUEAST S06 vista lateral" />
        ...
        <div className="desc__select" onClick={() => setOpen(!open)}>
          ...
          {open && (
            <ul className="desc__opts">
              {["1.5T-6DCT", "1.5T-6DCT Lite", "1.6TD-7DCT Pro Max"].map(v => (
                <li key={v} onClick={(e) => { e.stopPropagation(); setVersion(v); setOpen(false); }}>{v}</li>
              ))}
            </ul>
          )}
        </div>
        <p className="desc__lead">The S06 is a C-segment SUV tailored for the urban new youth, featuring bold and stylish design, cutting-edge technology and outstanding performance that redefines the nature of urban mobility.</p>
        <div className="desc__price">
          <span className="desc__priceLabel">Precio desde</span>
          <span className="desc__priceVal">$10.990.000<sup>*</sup></span>
        </div>
        <p className="desc__fine">*Lorem ipsum dolor sit amet, consectetur adipiscing elit. Praesent ipsum odio, suscipit at malesuada id, bibendum id lacus.</p>
      </div>
      <style>{`
...
const ColoresSection = () => {
  ...
  const colors = [
    { name: "Aurora Green", hex: "#92B1B9" },
    { name: "Obsidian Black", hex: "#1E1E1E" },
    { name: "Pearl White", hex: "#F3F3F1" },
    { name: "Sunset Orange", hex: "#D9845B" },
    { name: "Ocean Blue", hex: "#3A6F8F" },
    { name: "Crimson Red", hex: "#A03830" },
    { name: "Stellar Silver", hex: "#BDBCBC" },
  ];
  ...
  return (
    <section className="col" id="colores">
      ...
      <div className="col__grid">
        <div className="col__main" style={{ backgroundImage: "url('assets/modelo-color-exterior.jpg')" }} role="img" aria-label={`${colors[picked].name} — exterior`} />
        <div className="col__side" style={{ backgroundImage: "url('assets/modelo-color-interior.jpg')" }} role="img" aria-label={`${colors[picked].name} — interior`} />
      </div>
      <div className="col__swatchRow">
        <span className="tick" style={{ marginRight: 16 }} />
        <p className="col__swatchName">{colors[picked].name}</p>
        <div className="col__swatches">
          {colors.map((c, i) => (
            <button key={c.name} onClick={() => setPicked(i)} className={"col__swatch" + (picked === i ? " is-active" : "")} style={{ background: c.hex }} aria-label={c.name} />
          ))}
        </div>
      </div>
      <style>{`
...
const GaleriaSection = () => {
  const slides = [
    { feat: "Pantalla de 15,6\"", desc: "Una pantalla inmersiva que da vida a cada momento con claridad cinematográfica." },
    { feat: "Aire acondicionado automático de doble zona", desc: "Climatización independiente para conductor y acompañante." },
    { feat: "Ajuste electrónico de los asientos delanteros", desc: "Posición perfecta al toque de un botón." },
    { feat: "Apple CarPlay & Android Auto", desc: "Conecta tu mundo digital sin esfuerzo." },
  ];
  ...
  return (
    <section className="gal" id="galeria">
      ...
      <div className="gal__stage">
        ...
        <div className="gal__img" style={{ backgroundImage: "url('assets/modelo-gallery.jpg')" }} role="img" aria-label={`galería — ${slides[idx].feat}`} />
...
const SeguridadSection = () => {
  const items = [
    { title: "ICA (Asistente de Crucero Integrado)", desc: "Mantiene la posición en el carril y ajusta la velocidad sin esfuerzo, aliviando la carga del conductor para una conducción más segura y cómoda." },
    { title: "HMA (Asistente de Luces de Carretera)", desc: "Mejora la visibilidad y la seguridad, alternando entre luces largas y cortas según sea necesario." },
    { title: "LKA (Asistente de Mantenimiento de Carril)", desc: "Reduce la desviación del carril y mejora la seguridad para una experiencia de conducción más segura y sin estrés." },
  ];
  return (
      ...
      <div className="seg__grid">
        {items.map((it, i) => (
          <article key={i} className="seg__card">
            <div className="seg__img" style={{ backgroundImage: "url('assets/seguridad.jpg')" }} role="img" aria-label={it.title} />
            <h3 className="seg__title">{it.title}</h3>
...
const SpecsTable = () => {
  ...
  const rows = [
    ["Largo x Ancho x Alto (mm)", "4616 x 1910 x 1685", "4616 x 1910 x 1685", "4616 x 1910 x 1690"],
    ["Distancia entre ejes", "2720", "2720", "2720"],
    ["Altura mínima al suelo (mm)", "160/190", "160/190", "160/190"],
    ["Peso en vacío (kg)", "1512", "1512", "1580"],
    ["Cilindrada (ml)", "1498", "1498", "1598"],
    ["Potencia neta máxima (kW) @ rpm", "115@5500", "115@5500", "145@5500"],
    ["Torque máximo (Nm) @ rpm", "230@1750-4000", "230@1750-4000", "290@2000-4000"],
    ["Neumáticos", "235/60 R18", "235/60 R18", "255/45 R20"],
  ];
...
const CotizarSection = () => {
  ...
  // Ref del timeout del banner "sent" — limpiamos al desmontar para evitar
  ...
  // Bloomreach — form_view (una vez al montar la sección)
  React.useEffect(() => {
    if (window.bloomreachService) {
      window.bloomreachService.trackEvent("form_view", { form: "cotizador_s06" });
    }
  }, []);
  ...
  const submit = async (ev) => {
    ...
    // Bloomreach — form_submit (intento de envío, antes del POST)
    if (window.bloomreachService) {
    ...
    setSubmitting(true);
    try {
      const result = await window.cotizadorService.enviarCotizacion(form, { modelo: "SOUEAST S06" });
      if (result.ok) {
        setSent(true);
        if (sentTimeoutRef.current) clearTimeout(sentTimeoutRef.current);
        sentTimeoutRef.current = setTimeout(() => setSent(false), 5000);

        // Bloomreach — identifica al cliente con su email + lead_sent
        if (window.bloomreachService) {
        ...
        if (window.bloomreachService) {
          window.bloomreachService.trackEvent("lead_error", {
            form: "cotizador_s06",
      ...
      console.error("[CotizarSection] submit error", err);
    ...
    <section className="cot" id="cotizar">
      <div className="cot__inner">
        <div className="cot__intro">
          ...
          <h2 className="cot__title">Cotiza tu SOUEAST S06</h2>
